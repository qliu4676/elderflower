    ...
    if method == "Galsim":
        ...
        if not parallel:
            ...
            image_real = np.sum([m2d(xx,yy) + p2d(xx,yy) 
                                for (m2d,p2d) in zip(moffat2d_s[bright], power2d_s[bright])], axis=0)
        else:
            print("Rendering bright stars in parallel...")
            func2d_s = np.concatenate([moffat2d_s[bright], power2d_s[bright]])
...
def plot_fitting_vs_truth_PSF(res, true_pars, image_size=image_size, 
                              n_bootsrap=100, save=True, dir_name="."):
    from dynesty import utils as dyfunc
